...
function GalleryButtonRequestQuote(id) {
    ...
    let product = GalleryWithButton.Products.filter(p => p.id === String(id))[0];
    ...
    let form = jQuery("#gallery-with-quote-contact-form-div");
    product_name.textContent = JSON.parse(product.name);
    product_desc.textContent = JSON.parse(product.desc);
    product_price.textContent = JSON.parse(product.price);
    form.show();
}